...
Want to see what the output looks like before running the tool? A sanitized
example (using placeholder account data) lives in [`sample-reports/`](sample-reports/):

- **▶ [View the rendered HTML report](https://aws-samples.github.io/sample-wa-foundations-assessment/sample-reports/wafa-report.html)** — live, interactive preview (GitHub Pages)
- [`wafa-report.html`](sample-reports/wafa-report.html) — the HTML report source
- [`wafa-checks.csv`](sample-reports/wafa-checks.csv) — the CSV export
- [`wafa-raw.json`](sample-reports/wafa-raw.json) — the raw JSON

> The GitHub file links above show the HTML/CSV/JSON as raw source. Use the
> **View the rendered HTML report** link to see the report as an actual page.

The sample models a management account with a realistic mixed posture (most
foundations in place, a few gaps, and one permission error). It is generated by
the real report code via [`scripts/generate_sample_report.py`](scripts/generate_sample_report.py),
so it stays in sync with the tool's actual output. Regenerate it with
